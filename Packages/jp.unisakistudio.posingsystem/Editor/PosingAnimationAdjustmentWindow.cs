...
{
    using static PosingAnimationAdjustmentMuscle;
    public class PosingAnimationAdjustmentWindow : EditorWindow
    {
        ...
        private readonly Dictionary<PosingAnimationAdjustmentMuscleGroupName, bool> _muscleFoldoutStates = new();
            ...
            using (new EditorGUI.DisabledScope(!enabled))
            {
                ...
                using (var scroll = new EditorGUILayout.ScrollViewScope(_muscleScroll))
                {
                    ...
                    foreach (var group in muscleGroups)
                    {
                        ...
                        if (!_muscleFoldoutStates.TryGetValue(group.Name, out var expanded))
                        {
                            expanded = true;
                            _muscleFoldoutStates[group.Name] = true;
                        }

                        DrawMuscleGroupFoldout(group, ref expanded);
                        _muscleFoldoutStates[group.Name] = expanded;
                        if (!expanded)
                        {
                            continue;
                        }
                        ...
                        foreach (var muscleIndex in group.MuscleIndices)
                        {
        ...
        private void DrawMuscleGroupFoldout(PosingAnimationAdjustmentMuscleGroup group, ref bool expanded)
        {
            var rect = EditorGUILayout.GetControlRect();
            var controlRectWidth = rect.width;
            var controlRectDefaultX = rect.x;
            const float buttonWidth = 20f;

            if (group.HasOpposite)
            {
                rect.width = EditorGUIUtility.labelWidth;
            }

            expanded = EditorGUI.Foldout(rect, expanded, group.DisplayName, true);

            if (group.HasOpposite && expanded)
            {
                ...
                if (pressed)
                {
                    var oppositeGroup = MuscleGroups[(int)group.OppositeName];
                    CopyGroupStateFromOpposite(group, oppositeGroup);
                    SaveCurrentAdjustmentData();
                    ApplyPreviewPose();
                }
            }
        ...
        private void CopyGroupStateFromOpposite(PosingAnimationAdjustmentMuscleGroup dstGroup, PosingAnimationAdjustmentMuscleGroup srcGroup)
        {
            Undo.RecordObject(_adjustments, $"Copy {dstGroup.DisplayName} from {srcGroup.DisplayName}");

            for (int i = 0; i < dstGroup.MuscleIndices.Count; i++)
            {
                var dstMuscleIndex = dstGroup.MuscleIndices[i];
                var srcMuscleIndex = srcGroup.MuscleIndices[i];

                _adjustments.adjustments[dstMuscleIndex] = _adjustments.adjustments[srcMuscleIndex];
                SetLimitBreakState(dstMuscleIndex, GetLimitBreakState(srcMuscleIndex));
            }
        }

        private void ClearGroupState(PosingAnimationAdjustmentMuscleGroup group)
        {
            if (!_adjustments)
            {
                return;
            ...
            Undo.RecordObject(_adjustments, $"Reset {group.DisplayName}");

            for (int i = 0; i < group.MuscleIndices.Count; i++)
            {
                var muscleIndex = group.MuscleIndices[i];
                _adjustments.adjustments[muscleIndex] = 0f;
                SetLimitBreakState(muscleIndex, false);
            }
        }
